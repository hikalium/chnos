...
void API_Execute(uint edi, uint esi, uint ebp, uint esp, uint ebx, uint edx, uint ecx, uint eax)
{
	...
	if(edx == 0x00000001){
		...
		InputBox_Put_String(nowtask->cons->input, (uchar *)(ebx + System_SegmentDescriptor_Get_Base(nowtask->cons->app_cs)));
	} else if(edx == 0x00000003){
		for(i = 0; i < ecx; i++){
			InputBox_Put_Character(nowtask->cons->input, ((uchar *)ebx)[i]);
		}
	} else{
